...
async function testFixedWebhook() {
  console.log('🔍 Testing fixed webhook with real Cryptomus data...');
  
  // Real webhook data from logs
  const webhookData = {
    "type": "payment",
  ...
  try {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        console.log('📋 Could not read admin notifications log:', logError.message);
      }
    }, 2000);
