...
// Set global flag to indicate webhook context
...
export default async function handler(
  ...
  try {
    ...
    try {
      ...
      console.log('✅ NEW WEBHOOK - Logged and processing:', webhookId);
    } catch (webhookLogError: any) {
      console.error('❌ Error managing webhook log:', webhookLogError);
      // Continue processing but log the error
      const fs = require('fs');
      const errorEntry = `${new Date().toISOString()} - WEBHOOK LOG ERROR: ${webhookLogError.message}\n`;
    ...
    if (status === 'paid' || status === 'paid_over') {
      ...
      if (existingOrder) {
        ...
        try {
          console.log('🔔 PAYMENT CONFIRMED (EXISTING ORDER) - Sending notification for:', updatedOrder.id);
          ...
          // Skip notification - will be handled by main webhook in pages/api/pay/cryptomus/webhook.ts
          console.log('⚠️ PAYMENT CONFIRMED (EXISTING ORDER) - Notification skipped to prevent duplicates for:', updatedOrder.id);
          
          // Log skip to file
          const skipEntry = `${new Date().toISOString()} - PAYMENT CONFIRMATION NOTIFICATION SKIPPED (DUPLICATE PREVENTION): ${updatedOrder.id}\n`;
          fs2.appendFileSync('logs/webhook-debug.log', skipEntry);
        ...
        try {
          ...
          // Skip notification - will be handled by main webhook in pages/api/pay/cryptomus/webhook.ts
          console.log('⚠️ PAYMENT CONFIRMED - Notification skipped to prevent duplicates for:', order.id);
          
          // Log skip to file
          const skipEntry = `${new Date().toISOString()} - PAYMENT CONFIRMATION NOTIFICATION SKIPPED (DUPLICATE PREVENTION): ${order.id}\n`;
          fs2.appendFileSync('logs/webhook-debug.log', skipEntry);
